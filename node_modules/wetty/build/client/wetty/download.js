...
async function onCompleteFile(bufferCharacters) {
  ...
  const fileName = `file-${new Date().toISOString().split(".")[0].replace(/-/g, "").replace("T", "").replace(/:/g, "")}${fileExt ? `.${fileExt}` : ""}`;
...
export class FileDownloader {
  ...
  async bufferCharacter(character) {
    ...
    if (this.fileBuffer.length >= this.fileBegin.length + this.fileEnd.length && this.fileBuffer.slice(-this.fileEnd.length).join("") === this.fileEnd) {
      await this.onCompleteFileCallback(this.fileBuffer.slice(this.fileBegin.length, this.fileBuffer.length - this.fileEnd.length).join(""));
      this.fileBuffer = [];
    }
    return "";
  }
