...
class Feeds:
    @staticmethod
    def get_articles(feed_url, days=1):
        now_struct = time.gmtime()
        # 24 hours ago as struct_time
        cutoff_struct = time.gmtime(time.mktime(now_struct) - 86400*days)
        articles = []
        feed = feedparser.parse(feed_url, agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/139.0.0.0 Safari/537.36",request_headers={'Accept': 'application/rss+xml, application/xml, text/xml'})
        for entry in feed.entries:
            ...
            if 'tldr' in feed_url:
                summ = requests.get(entry.link).text
                print(summ)
